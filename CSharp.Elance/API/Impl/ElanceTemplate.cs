...
{
    /// <summary>
    /// This is the central class for interacting with Elance.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Some Elance operations require OAuth authentication. 
    /// To perform such operations, <see cref="ElanceTemplate"/> must be constructed 
    /// with the minimal amount of information required to sign requests to Elance's API 
    /// with an OAuth <code>Authorization</code> header.
    /// </para>
    /// <para>
    /// There are some operations that do not require OAuth authentication. 
    /// In those cases, you may use a <see cref="ElanceTemplate"/> that is created through 
    /// the default constructor and without any OAuth details.
    /// Attempts to perform secured operations through such an instance, however, 
    /// will result in <see cref="ElanceApiException"/> being thrown.
    /// </para>
    /// </remarks>
    /// <author>Scott Smith</author>
    public sealed class ElanceTemplate : AbstractOAuth2ApiBinding, IElance 
    {
        private static readonly Uri ApiUriBase = new Uri("https://api.elance.com/api2/");

        /// <summary>
        /// Create a new instance of <see cref="ElanceTemplate"/> able to perform unauthenticated operations against Elance's API.
        /// </summary>
        /// <remarks>
        /// Some operations do not require OAuth authentication. 
        /// A ElanceTemplate created with this constructor will support those operations. 
        /// Any operations requiring authentication will throw an <see cref="ElanceApiException"/>.
        /// </remarks>
        public ElanceTemplate()
            : base()
        {
            InitSubApis();
        }
        ...
        /// <summary>
        /// Create a new instance of <see cref="ElanceTemplate"/>.
        /// </summary>
        /// <param name="accessToken">An access token acquired through OAuth authentication with Elance.</param>
        public ElanceTemplate(string accessToken)
            : base(accessToken)
        {
            InitSubApis();
        }

        #region IElance Members

        /// <summary>
        /// Gets the underlying <see cref="IRestOperations"/> object allowing for consumption of Elance endpoints 
        ...
        protected override void ConfigureRestTemplate(RestTemplate restTemplate)
        {
            ...
            restTemplate.ErrorHandler = new ElanceErrorHandler();
        }

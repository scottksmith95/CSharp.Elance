...
namespace Spring.Social.OAuth2
{
    ...
    public class OAuth2Template : IOAuth2Operations
    {
        ...
        {
            string accessToken = response.GetValue("data").GetValue<string>("access_token");
            string scope = response.GetValue("data").GetValueOrDefault<string>("scope");
            string refreshToken = response.GetValue("data").GetValueOrDefault<string>("refresh_token");
            int? expiresIn = null;
            try { expiresIn = response.GetValue("data").GetValueOrDefault<int?>("expires_in"); }
            catch (JsonException) { }
